...
import { getAllowPoints } from '../helper'
import ChessPiece from '../libs/ChessPiece'
import Point from '../libs/Point'
import { Camp } from '../definitions'
import { ChessPieceValue } from '../types/chessPiece'
import { P, isArrayEqual, newPieceByPoint } from './helper'
...
suite('move list', () => {
  ...
  describe.each(list)('$text', ({ value, arr }) => {
    const piece = new ChessPiece(value as ChessPieceValue, 0)

    test.each(arr)('%o 无阻碍移动', (p, asserted) => {
      ChessPiece.move(piece, p as Point)

      isArrayEqual(getAllowPoints(piece, []), asserted as Point[])
    })

    // 车和炮，需要特殊处理
    if (![12, 22, 14, 24].includes(value)) {
      test.each(arr)(`${value} - %o 已方阻碍移动`, (p, asserted) => {
        ChessPiece.move(piece, p as Point)
        ...
        for (let i = 0; i < len; i++) {
          ...
          isArrayEqual(
            getAllowPoints(piece, [piece, newPieceByPoint(p1, piece.camp)]),
            arr2
          )
        }
      })
      ...
      test.each(arr)('%o 敌方位置可移动', (p, asserted) => {
        ...
        for (let i = 0; i < len; i++) {
          ...
          isArrayEqual(
            getAllowPoints(piece, [piece, newPieceByPoint(p1, piece.camp === Camp.BLACK ? Camp.RED : Camp.BLACK)]),
            asserted as Point[]
          )
        }
      })
    ...
    if (value === 13 || value === 23) {
      test.each(arr)('%o 马脚被卡不可移动', (p, _asserted) => {
        ...
        // 针对当前所在位置增加马脚位
        isArrayEqual(getAllowPoints(piece, [piece, ...pList.map(point => newPieceByPoint(point, Camp.RED))]), [])
      })
    }
    ...
    if (value === 12 || value === 22) {
      test.each(arr)('%o 车被堵路', (p, asserted) => {
        ...
        isArrayEqual(getAllowPoints(piece, [piece, ...blockPieces]), (asserted as Point[]).filter(({ x, y }) => (
          // 已方不包含
          x < pRight.x &&
          y < pBottom.y &&
    ...
    if (value === 14 || value === 24) {
      test.each(arr)('%o 炮台就绪，开火', (p, asserted) => {
        ...
        isArrayEqual(getAllowPoints(piece, [piece, ...blockPieces, ...killPieces]), [...(asserted as Point[]).filter(({ x, y }) => (
          x < pRight.x &&
          y < pBottom.y &&
          x > pLeft.x &&
          y > pTop.y
